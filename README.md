...
**שימו לב: הגשת המטלות בקורס הינה רק ביחידים**

גרפים הם חלק בלתי נפרד מתכנית הלימודים שלכם. במהלך הקורס נחשפתם לדרכים שונות לייצוג גרפים (כגון מטריצת שכנויות, רשימת שכנויות ועוד). במטלה זו תייצגו גרף באמצעות רשימת שכנויות
(למידע נוסף: Adjacency list – Wikipedia).

המטרה העיקרית היא להעמיק את ההבנה בשימוש ב-namespaces ובניהול זיכרון ב-++C, תוך התמודדות עם אלגוריתמים גרפיים שונים – כולל טיפול במשקולות (weights) שמייצגות את עלות הצלעות.

להלן דרישות המטלה: 

* מחלקה בשם ``Graph`` שתכיל את רשימת השכנויות עצמה ומתודות הבאות:
